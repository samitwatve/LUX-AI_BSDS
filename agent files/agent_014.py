                ...
                if cell.has_resource():
                    if cell.resource.type == Constants.RESOURCE_TYPES.URANIUM:
                        if player.researched_uranium(): 
                            patch_resource_count += cell.resource.amount
        ...
        if 0 < patch[2] <= 5:
            ...
            for x1 in range(patch[0][0], patch[0][0]+4):
                for y1 in range(patch[0][1], patch[0][1]+4):
                    cell = game_state.map.get_cell(x1, y1)
                    if len(perimeter_tiles) > 0:
                        if cell.citytile is None and cell.has_resource() == False and cell in perimeter_tiles:
                            build_locations.append(cell)
                    elif cell.citytile is None and cell.has_resource() == False:
                        build_locations.append(cell)
...
def unit_actions_policy(player, actions):
    ...
    resource_tiles = find_resources(game_state)
    #for rt in resource_tiles[0:3]:
        #print(rt.resource.amount)
