...
def ideal_build_locations(perimeter_tiles, city_adjacent_tiles, player):
    build_locations = []

    if perimeter_tiles is not None:
        if city_adjacent_tiles is not None:
            for p_tile in perimeter_tiles:
                for c_a_tile in city_adjacent_tiles:
                    if p_tile.pos.equals(c_a_tile.pos):
                        build_locations.append(c_a_tile)
                        
    if len(build_locations) == 0:
        if perimeter_tiles is None:
            build_locations = city_adjacent_tiles
        if city_adjacent_tiles is None:
            build_locations = perimeter_tiles
...
def worker_actions_policy(player, actions):
    ...
    city_adjacent_tiles = find_city_adjacent_empty_tiles(player)
...
#         else:
        ...
        build_locations = ideal_build_locations(perimeter_tiles, city_adjacent_tiles, player)
        ...
        if unit.is_worker() and unit.can_act():
            ...
            if time_to_darkness > unit.pos.distance_to(closest_city_tile.pos):
                ...
                if closest_city_tile_needing_fuel is None:
                    ### TRY TO BUILD CITIES
                    for build_location in build_locations:
                        if unit.pos.equals(build_location.pos):                        
                    ...
                    ### OR MOVE TO A RANDOM SPOT
                            ...
                            if not unit_will_collide(player, unit, moving_to):
                                spot = find_closest_build_location(unit.pos, player, build_locations).pos
                                action = unit.move(unit.pos.direction_to(spot))
